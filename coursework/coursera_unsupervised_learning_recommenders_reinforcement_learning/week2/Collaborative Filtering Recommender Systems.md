...
|<b>General </b>  Nwotation  | Description| Python (if any) |
|-------------|----------------|--------------------------------------------|
...
![[Pasted image 20221122205645.png]]
...
![[Pasted image 20221122205706.png]]
...
def cofi_cost_func(X, W, b, Y, R, lambda_):
    ...
    for j in range(nu):
        for k in range(n):
            J += (lambda_ / 2) * np.square(W[j][k])

    for i in range(nm):
        for k in range(n):
            J += (lambda_ / 2) * np.square(X[i][k])
